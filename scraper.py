...
def buscar_arena(produto):
    ...
    for card in resultados:
        ...
        # Peso: pode vir no nome ou no span de peso médio
        peso_kg = extrair_peso(nome)
...
def montar_carrinho(supermercado, produtos_lista, func_scraper):
    ...
    for produto in produtos_lista:
        try:
            ...
            print(f"[ERRO] {supermercado} - {produto}: {e}")
            carrinho["faltando"] += 1
...
def main():
    ...
    # Goodbom
    try:
        resultados["Goodbom"] = montar_carrinho("Goodbom", produtos_lista, buscar_goodbom)
    except Exception as e:
        print(f"[ERRO GERAL] Goodbom: {e}")
        resultados["Goodbom"] = {"produtos": [], "total": 0.0, "faltando": len(produtos_lista)}

    # Arena
    try:
        resultados["Arena Atacado"] = montar_carrinho("Arena Atacado", produtos_lista, buscar_arena)
    except Exception as e:
        print(f"[ERRO GERAL] Arena: {e}")
        resultados["Arena Atacado"] = {"produtos": [], "total": 0.0, "faltando": len(produtos_lista)}
